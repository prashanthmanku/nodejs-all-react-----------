...
app.get("/user/tweets/", authentication, async (request, response) => {
  ...
  const getTweetsLikesrepliesCount = `
        SELECT *,(
        ...
        FROM tweet
        WHERE tweet.user_id=${dbLoginUser.user_id};
  `;
  const finaltTweetsCounts = await db.all(getTweetsLikesrepliesCount);
  console.log(finaltTweetsCounts);
  const r = `select * from like where tweet_id=4;`;
  const rr = await db.all(r);
  console.log(rr);
  response.send(finaltTweetsCounts);
});
//post Tweet API
